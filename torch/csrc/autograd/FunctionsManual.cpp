...
void print_shape(const Tensor& x, const std::string& msg) {
  auto sizes = x.sizes();
  std::cout << msg << ": ";
  for (auto i : sizes) {
    std::cout << i << " ";
...
Tensor mul_tensor_backward(const Tensor& grad, T other, ScalarType self_st) {
  ...
  // print_shape(grad, "grad")
  // print_shape(other, "other")
  //! here we found out that shape of out is same as shape of grad in BERT, so
  //! we can return grad directly
  return grad;
...
Tensor mm_mat1_backward(
  ...
  //! emu
  //! we assume both grad and mat1 are 2-d matrixs
  auto gsize = grad.sizes();
  auto mat2size = mat2.sizes();
  auto out = at::zeros({gsize[0], mat2size[0]}, grad.options());
  return out;
  // print_shape(grad, "mm_mat1_bk grad");
  // print_shape(mat2, "mm_mat1_bk mat2 ");
  // auto out = grad.mm(mat2.t());
  // print_shape(out, "mm_mat1_bk out");
...
Tensor mm_mat2_backward(
  ...
  //! emu
  // print_shape(grad, "mm_mat2_bk grad");
  // print_shape(mat1, "mm_mat2_bk mat1");
  // auto out = mat1.t().mm(grad);
  // print_shape(out, "mm_mat2_bk out");
